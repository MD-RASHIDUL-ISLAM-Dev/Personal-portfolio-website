(() => {
  ...
  const projects = {
    archive: { label: 'MRI PDF Archive Vault · Live Bot', promise: 'You will learn to turn a messy document library into a searchable system.', href: 'https://t.me/MRI_PDF_ARCHIVE_Bot', art: 'project-mri-vault-relatable.webp', name: 'MRI PDF Archive Vault', tag: 'DOCUMENT SYSTEMS', kicker: 'Deployment portrait' },
    deepseek: { label: 'DeepSeek Telegram AI Bot · Conversational AI', promise: 'You will learn to connect a powerful model to a focused user experience.', href: '#projects', art: 'project-deepseek-telegram-relatable.webp', name: 'DeepSeek Telegram AI Bot', tag: 'CONVERSATIONAL AI', kicker: 'System portrait' },
    portfolio: { label: 'Personal Portfolio Website · Live Demo', promise: 'You will learn to make technical work feel clear, calm, and memorable.', href: 'https://www.mdrashidulislam.kdns.fr/', art: 'project-portfolio-relatable.webp', name: 'Personal Portfolio Website', tag: 'FRONTEND SYSTEMS', kicker: 'Interface portrait' },
  };
  ...
  const renderGithub = async () => {
    ...
    try {
      ...
      if (timeline) timeline.innerHTML = publicActivity.slice(0, 5).map(event => `<div class="github-timeline__item"><span>${event.type.replace('Event','')}</span><small>${new Date(event.created_at).toLocaleDateString()}</small></div>`).join('') || '<p class="small-note">No recent public activity returned.</p>';
  ...
  const comparisonData = {
    archive: { problem: 'Document retrieval', stack: 'Telegram · Python · SQLite', status: 'Shipped', proof: 'Live bot + public source' },
    deepseek: { problem: 'Conversational AI in Telegram', stack: 'DeepSeek · OpenRouter · Telegram', status: 'In progress', proof: 'Active build plan' },
    portfolio: { problem: 'Clear technical positioning', stack: 'HTML · CSS · JavaScript', status: 'Live', proof: 'Live site + public source' }
  };
  $('#project-compare-select')?.addEventListener('change', event => { const data = comparisonData[event.target.value]; if (!data) return; Object.entries(data).forEach(([field, value]) => { const node = $(`#compare-${field}`); if (node) node.textContent = value; }); });

  /* Fit quiz: deterministic recommendation, no invented estimate */
  $('#fit-quiz-form')?.addEventListener('submit', event => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const goal = String(form.get('goal') || 'bot');
    const stage = String(form.get('stage') || 'idea');
    const suggestions = {
      bot: ['Telegram AI Bot service', 'MRI PDF Archive Vault case study'],
      llm: ['LLM Integrations service', 'DeepSeek Telegram AI Bot case study'],
      automation: ['Python Automation service', 'MRI PDF Archive Vault case study'],
      web: ['Responsive Web Interfaces service', 'Personal Portfolio Website case study']
    };
    const match = suggestions[goal] || suggestions.bot;
    const result = $('#fit-quiz-result');
    if (result) result.textContent = `${match[0]} is the clearest starting point for a ${stage} project. Explore the ${match[1]} or send a brief to discuss scope.`;
    document.dispatchEvent(new CustomEvent('rifat:track', { detail: { name: 'fit_quiz_complete', label: goal } }));
  });
  ...
  if ('IntersectionObserver' in window && sectionBeaconLabel) { const spy = new IntersectionObserver(entries => entries.forEach(entry => { if (entry.isIntersecting) { sectionBeaconLabel.textContent = sectionNames.get(entry.target.id) || entry.target.id; $$('.site-nav__links a').forEach(link => link.classList.toggle('is-current', link.getAttribute('href') === `#${entry.target.id}`)); } }), { rootMargin: '-35% 0px -55% 0px', threshold: 0 }); sectionsForSpy.forEach(section => spy.observe(section)); }
