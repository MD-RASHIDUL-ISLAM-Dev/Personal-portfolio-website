...
## v25.3 Overflow containment repair

The v25.3 patch repairs the fixed header and long-copy containment after the AI Studio expansion. Desktop navigation now uses shrinkable children and controlled gaps so the brand, primary links, quick social links and utility controls remain inside the viewport. Contact forms, social links, consent copy, AI outputs and route cards now use `min-width: 0`, `max-width: 100%` and safe wrapping. Mobile navigation keeps the existing menu, body-lock, ARIA state and five utility controls. The repair was verified at desktop and 390px mobile widths in light, dark, Bengali and restored English states.


## v25.5 Mobile AI Studio composition repair

The v25.5 layout pass is matched to the reported narrow-screen rendering. AI Studio now uses a single-column flow through 1100px, card height follows natural content instead of stretched grid rows, output blocks do not reserve unnecessary empty space, and consent text remains horizontal and contained beside a fixed-size checkbox. The fixed navigation uses the compact menu through the same 1100px breakpoint, moving the five utility controls into the menu and preventing the brand, links and controls from competing for one row. Fresh 390px and 360px audits reported zero card/header descendants outside the viewport.

## v27 Feature Command Center

The v27 release adds an audience-aware proof surface without introducing fabricated claims. Visitors can select a local intent path for recruiter, client, developer or general exploration. The Interactive Project Evidence Room switches among the MRI Vault, DeepSeek Bot and Portfolio, exposing architecture, technical decisions, failure boundaries, security notes and next proof paths, with a Markdown one-page export. The Technical Decision Log makes the trade-offs behind the systems explicit.

The Deployment Health Panel reads the configured protected status endpoint when one exists and otherwise remains explicitly unconnected. The Privacy & AI Transparency card explains local storage, provider boundaries and the no-fabricated-metrics policy. Content Freshness identifies the manually curated v27.0 source release, while the Evidence Pack action provides a portable summary. Accessibility controls link directly to reduced motion, high contrast, larger text, focus mode and the keyboard guide already present in the static site. All new copy includes Bengali/English data attributes and the mobile layout remains single-column without horizontal overflow.

## v28 Professional Delivery Layer

The v28 release turns the portfolio from a feature showcase into a clearer collaboration and delivery surface. The Client Collaboration Charter explains the working agreement from goal and build through review and handoff. The Contact Preflight Checker tests whether a first message has a goal, platform and deadline before the real form is opened, entirely locally. The Risk & Trade-off Matrix and Security Threat Model Viewer make constraints, failure boundaries, mitigations and limitations visible without presenting them as formal production scores or certifications.

The Before/After Workflow Simulator demonstrates the MRI Vault value locally without contacting Telegram. The Claim-to-Proof Ledger connects Python automation, LLM integration, frontend systems and fuzzy search to relevant proof paths. The Project Handoff Preview shows README, environment, recovery and next-step artifacts. The Design System Explorer exposes local tone and text-scale changes, while Release Verification and Shareable Project Routes provide static-safe source and navigation signals. All new feature copy supports Bengali/English localization, light-mode contrast and reduced-motion-safe behavior.

## v29 Proof Lab / Traceable Evidence Layer

The v29 release adds a deeper proof and maintenance layer. The Evidence Confidence Board distinguishes live bot, active build and verified static proof. The Interactive Architecture Trace exposes interface, orchestration, state, intelligence and fallback boundaries. The Live Demo Sandbox demonstrates curated fuzzy-search, backup and provider-fallback states locally without contacting Telegram or a model provider.

The Performance Budget Panel reports only browser-observed resource, image, script, style, transfer and connection information, and explicitly avoids presenting Lighthouse or Core Web Vitals claims. The Security Disclosure explains what may be public, what must remain behind a protected boundary and what a connected service needs before activation. The Maintenance Preview, Personalized Proposal Generator, Release Diff Viewer, Proof Rail, Why This Stack rationale and Case Study Citation Anchors make the portfolio easier to evaluate, maintain and discuss without fabricating prices, activity or certification.

## v30 Guided Evaluation Layer

The v30 release adds a guided review surface for recruiters, clients and developers. The Guided Portfolio Tour presents role-specific proof sequences. Interactive Technical Interview Mode exposes documented answers around SQLite, LLM failure and static delivery. The Request/Response Playground demonstrates local integration contracts without external requests. The Compatibility Matrix records the verified mobile, keyboard, reduced-motion, offline-shell and optional endpoint states without presenting a universal certification.

The Proof Lineage Viewer connects a claim to its case study, source and QA evidence. The Acceptance Criteria Builder turns a selected behavior into a local checklist. The Release Fingerprint computes a browser-side SHA-256 of the current HTML response and clearly distinguishes it from signed package attestation. Assistant notes can be exported as Markdown, projects can be shortlisted locally, deployment environments can be compared, build decisions can be replayed, and the screen-reader reading order can be inspected. All new controls preserve the bilingual, light/dark, reduced-motion and static-hosting constraints.

## v31 Evaluation Operations Layer

The v31 release adds a self-auditing and conversion layer. The Project Comparison Board turns the saved project set into a side-by-side proof view. The Self-health Scanner checks local IDs, images, alt text, internal anchors and basic metadata without treating external network state as verified. The Real Performance Observer reports browser-observed paint, resource, layout-shift support and navigation timing only when available.

The SEO & Social Preview Studio previews share language without rewriting live metadata. Interview Rubric Mode exposes reasoning dimensions without fabricating a score. The Client Discovery Meeting Builder prepares questions, unknowns and a first milestone. The What Changed panel uses a local release marker, the Public Roadmap separates shipped, next and rejected work, and the Asset Health Panel checks local image sources, dimensions and alt text.

The Private Inquiry Draft Vault saves an unfinished brief only in the browser. Case Study Branching Replay provides security-first, speed-to-market and maintenance-first lenses. Accessibility Scenario Simulator combines existing controls into readable test states. Each new feature retains bilingual rendering, light/dark readability, reduced-motion compatibility and static-hosting-safe fallbacks.

## v32 Mobile UX Layer

The v32 release improves the long single-page portfolio for narrow screens without removing its deeper proof. A Mobile Quick Start block gives direct routes to selected work, technical proof, résumé and contact. Advanced proof paths are progressively disclosed. Touch targets, visible focus rings, project rail snapping, safe-area padding, typography and the AI bottom sheet are refined for mobile use.

Fit Quiz, project brief and contact forms now use a mobile step flow with one active step, live progress, previous/next navigation and a submit control exposed at the final step. The browser-only draft vault and offline fallback remain local and clearly labeled. Non-critical images receive lazy-loading and async-decoding hints. v32.1 specifically moves the floating AI launcher above the sticky mobile CTA so the two controls do not overlap at 390px or 360px.

## v32.2 Keyboard Guide Repair

The v32.2 patch repairs the Keyboard Guide initialization path. Its language listener now runs after language state is declared, avoiding an initialization error that could stop later handlers. The guide opens with a button click, moves focus to a focusable shortcut panel, closes with Escape or the trigger, returns focus to the trigger, and keeps its shortcut labels synchronized in English and Bengali. The panel remains contained on mobile and desktop.

## v32.5 Preview Section Repair

Project Preview actions no longer rely on hover to become visible. The three project-card Preview buttons are discoverable and touch-safe on mobile and remain visible on desktop. Each opens the existing case-study dialog with local artwork, correct project content and focus return. The embedded bot preview and workflow reel remain deterministic local simulations.
